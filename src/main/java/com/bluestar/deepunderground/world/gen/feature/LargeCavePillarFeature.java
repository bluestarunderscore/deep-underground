...
public class LargeCavePillarFeature extends Feature<LargeCavePillarFeatureConfig>
{
    private Block pillarBlock= Blocks.DRIPSTONE_BLOCK;
    public LargeCavePillarFeature(Codec<LargeCavePillarFeatureConfig> codec, Block pillarBlock)
    {
        super(codec);
    ...
    @Override
    public boolean generate(FeatureContext<LargeCavePillarFeatureConfig> context) {
        StructureWorldAccess structureWorldAccess = context.getWorld();
        BlockPos blockPos = context.getOrigin();
        LargeCavePillarFeatureConfig largeDripstoneFeatureConfig = context.getConfig();
        Random random = context.getRandom();
        if (!LargeCavePillarHelper.canGenerate(structureWorldAccess, blockPos)) {
            return false;
        } else {
            ...
            if (!optional.isEmpty() && optional.get() instanceof CaveSurface.Bounded) {
                ...
                if (bounded.getHeight() < 4) {
                    ...
                    LargeCavePillarFeature.PillarGenerator dripstoneGenerator = createGenerator(
                            blockPos.withY(bounded.getCeiling() - 1), false, random, k, largeDripstoneFeatureConfig.stalactiteBluntness,
                            largeDripstoneFeatureConfig.heightScale, this.pillarBlock);

                    LargeCavePillarFeature.PillarGenerator dripstoneGenerator2 = createGenerator(
                            blockPos.withY(bounded.getFloor() + 1), true, random, k, largeDripstoneFeatureConfig.stalagmiteBluntness,
                            largeDripstoneFeatureConfig.heightScale, this.pillarBlock);
                    LargeCavePillarFeature.WindModifier windModifier;
                    if (dripstoneGenerator.generateWind(largeDripstoneFeatureConfig) && dripstoneGenerator2.generateWind(largeDripstoneFeatureConfig)) {
                        windModifier = new LargeCavePillarFeature.WindModifier(blockPos.getY(), random, largeDripstoneFeatureConfig.windSpeed);
                    } else {
                        windModifier = LargeCavePillarFeature.WindModifier.create();
                    }
    ...
    private static LargeCavePillarFeature.PillarGenerator createGenerator(
            BlockPos pos, boolean isStalagmite, Random random, int scale, FloatProvider bluntness, FloatProvider heightScale, Block pillarBlock
    ) {
        return new LargeCavePillarFeature.PillarGenerator(pos, isStalagmite, scale, bluntness.get(random), heightScale.get(random), pillarBlock);
    }

    private void testGeneration(StructureWorldAccess world, BlockPos pos, CaveSurface.Bounded surface, LargeCavePillarFeature.WindModifier wind) {
        ...
        for (BlockPos.Mutable mutable = pos.withY(surface.getFloor() + 2).mutableCopy(); mutable.getY() < surface.getCeiling() - 1; mutable.move(Direction.UP)) {
            ...
            if (LargeCavePillarHelper.canGenerate(world, blockPos) || world.getBlockState(blockPos).isOf(Blocks.DEEPSLATE)) {
                world.setBlockState(blockPos, Blocks.CREEPER_HEAD.getDefaultState(), Block.NOTIFY_LISTENERS);
            }
        }
    }

    static final class PillarGenerator {
        ...
        PillarGenerator(BlockPos pos, boolean isStalagmite, int scale, double bluntness, double heightScale, Block pillarBlock) {
            this.pos = pos;
        ...
        boolean canGenerate(StructureWorldAccess world, LargeCavePillarFeature.WindModifier wind) {
            while (this.scale > 1) {
                ...
                for (int j = 0; j < i; j++) {
                    ...
                    if (LargeCavePillarHelper.canGenerateBase(world, wind.modify(mutable), this.scale)) {
                        this.pos = mutable;
                        return true;
                    }
        ...
        private int scale(float height) {
            return (int)LargeCavePillarHelper.scaleHeightFromRadius((double)height, (double)this.scale, this.heightScale, this.bluntness);
        }

        void generate(StructureWorldAccess world, Random random, LargeCavePillarFeature.WindModifier wind) {
                    ...
                    if (!(f > this.scale)) {
                        ...
                        if (k > 0) {
                            ...
                            for (int m = 0; m < k && mutable.getY() < l; m++) {
                                BlockPos blockPos = wind.modify(mutable);
                                if (LargeCavePillarHelper.canGenerateOrLava(world, blockPos)) {
                                    bl = true;
        ...
        boolean generateWind(LargeCavePillarFeatureConfig config) {
    ...
    static final class WindModifier {
        ...
        static LargeCavePillarFeature.WindModifier create() {
            return new LargeCavePillarFeature.WindModifier();
        }
